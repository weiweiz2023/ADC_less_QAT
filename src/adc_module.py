...
class Nbit_ADC(nn.Module):
    ...
    def _sample_and_save_data(self, input_data, output_data):
        ...
        # 所有ADC输入保存到同一个文件
        with open("./saved/hist_csvs/test_resnet20+mnist_pre_cal_gf_input.csv", "a") as f:
            np.savetxt(f, input_sample.cpu().numpy(), delimiter=",")
        
        # 所有ADC输出保存到同一个文件  
        with open("./saved/hist_csvs/test_resnet20+mnist_pre_cal_gf_output.csv", "a") as f:
    ...
    def forward(self, x):
        ...
        min_val = self.min_val_base * self.step_size
        max_val = self.max_val_base * self.step_size
        # min_val = min_val * self.step_size  # Uses existing tensor operations
        # max_val = max_val * self.step_size  # No new tensor creation
        #min_val = torch.tensor(min_val * self.step_size, device=y.device, dtype=y.dtype)
        #max_val = torch.tensor(max_val * self.step_size, device=y.device, dtype=y.dtype)
        y = y.clamp(min_val, max_val) 

        # Data saving with intelligent sampling
        if self.save and self.saved_points < self.max_save_points:
            self._sample_and_save_data(x, y)
        loss = torch.tensor(0.0, device=x.device)  # 确保loss在正确设备上且   

        if self.custom_loss:
            loss = torch.mean(abs(x - y)  ) 
...
#         # self.step_size = 1
#         # self.zero_point = 0
#         # *** 关键修改：使用register_buffer使参数可保存 ***
#         self.register_buffer('step_size', torch.tensor(1.0))
#         self.register_buffer('zero_point', torch.tensor(0.0))

#     def start_calibration(self):
#         """开始校准模式"""
#         print(f"ADC {getattr(self, 'adc_name', 'unknown')} 开始校准...")
#         self.calibrating = True
#         self.cal_data = []
...
#         """完成校准并计算参数"""
#         if not self.cal_data:
#             print(f"  警告: ADC {getattr(self, 'adc_name', 'unknown')} 没有收集到校准数据")
#             self.calibrating = False
#             return
            
#         all_data = torch.cat(self.cal_data)
#         total_points = all_data.numel()
#         print(f"  收集了 {len(self.cal_data)} 个批次，{all_data.numel()} 个数据点")
#         print(f"  数据范围: [{all_data.min():.4f}, {all_data.max():.4f}]")
#         print(f"  均值: {all_data.mean():.4f}, 标准差: {all_data.std():.4f}")
#         indices = torch.randperm(total_points)[:2000]
#         sample_data = all_data[indices]

#         data_range = torch.quantile(sample_data, 0.95) - torch.quantile(sample_data, 0)
#         step_size = data_range / (2**(self.bits-1))
#         step_size_clamped = torch.clamp(step_size, 1e-6, 1e2)
#        # self.zero_point = 0
#         # *** 关键修改：正确更新buffer ***
#         self.step_size.data.copy_(step_size_clamped)
#         self.zero_point.data.copy_(torch.tensor(0.0))
#         print(f"  计算得到 step_size: {self.step_size:.6f}")
...
#     def forward(self, x):
#         # 在校准模式下收集数据
#         if self.calibrating and len(self.cal_data) < 50:  # 限制每个ADC收集的批次数
#             self.cal_data.append(x.flatten().cpu().detach())
...
#         if (self.save):  #and (random.random() < 0.0001) 
#             print(f"Saving ADC inputs...")
#             with open("./saved/hist_csvs/test_resnet20+mnist_raw_input.csv", "a") as f:
#                 array_to_write = (x).flatten().cpu().numpy()
#                 np.savetxt(f, array_to_write, delimiter=",")
#             with open("./saved/hist_csvs/test_resnet20+mnist_raw_output.csv", "a") as f:
...
class gradientFilter(Function):
    ...
    def backward(ctx, grad_output):
        ...
        scale1 = torch.clamp(0.2 + torch.log(input_tens ) / 3, min=0, max=1.0)
        scale3 = torch.clamp(torch.abs(torch.sin(input_tens * torch.pi)) * 0.8 + 0.2, min=0, max=1.0)
        grad_out = torch.where(input_tens >= min_val, scale3 * grad_output, grad_output)
        grad_out = torch.where(input_tens >= max_val, scale1 * grad_output, grad_out)
